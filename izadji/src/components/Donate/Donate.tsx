...
import DonationProject, {DonationProjectProps} from "../DonationProject/DonationProject";
            ...
            <div className={styles.donation__picker}>
                <div className={styles.donation__picker__inner}>
                    ...
                    <div className={`${styles.dropdown} js-dropdown`}>
                        <button type={"button"} className={`${styles.dropdown__btn} js-dropdown-btn`} onClick={({target}) => customDropdown.toggleDropdown(target as Element)}>{selectedProject.projectName}</button>
                        <ul className={`${styles.dropdown__list} js-dropdown-list`}>
                            {donationProjects.map(project => (<li className={styles.dropdown__item} key={project.projectPageId}>
                                <button onClick={({ target}) => {
                                    customDropdown.selectItem(target as Element);
                                    setSelectedProject(project);
                                }} className={`${styles.dropdown__link} js-dropdown-link`}>{project.projectName}</button>
                            </li>))}
                        </ul>
                    </div>
